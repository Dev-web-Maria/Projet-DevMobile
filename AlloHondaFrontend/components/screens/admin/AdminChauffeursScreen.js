...
const AdminChauffeursScreen = ({ navigation }) => {
                ...
                { text: 'Annuler', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            // Correction de l'endpoint et du format body pour match avec le backend .NET
                            // Backend expects: PUT api/Chauffeur/UpdateStatus/{id} with [FromBody] string newStatus
                            const backendStatus = newStatus === 'approved' ? 'Disponible' : 'Refusé';

                            const response = await fetch(`${API_BASE}/api/Chauffeur/UpdateStatus/${id}`, {
                                method: 'PUT',
                                headers: {
                            ...
                            if (response.ok) {
                                setChauffeurs(prev => prev.map(c =>
                                    c.id === id ? { ...c, status: backendStatus } : c
                                ));
    ...
    const filteredChauffeurs = chauffeurs.filter(c => {
        ...
        if (filter === 'approved') return c.status === 'Disponible' || c.status === 'Occupe';
