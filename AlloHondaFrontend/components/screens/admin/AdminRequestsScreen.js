 ...
 const fetchDemandes = async () => {
  try {
   ...
   if (response.ok) {
    ...
    // L'API renvoie { success: true, demandes: [...] } ou directement [...] ?
    // Dans ChauffeurDemandeVisualisation (Step 111), c'est data.demandes si data.success
    if (data.success && Array.isArray(data.demandes)) {
     setDemandes(data.demandes);
    } else if (Array.isArray(data)) {
     setDemandes(data);
    } else {
     // Fallback si format inconnu, on met tableau vide pour éviter crash
     setDemandes([]);
     console.log('Format données inconnu:', data);
    }
   } else {
